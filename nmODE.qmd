...
d.  In @fig-5.1 you will see a slope field and the exact solution to the differential equation $x' = -0.5x$ with $x(0) = 6$. Mark your approximate solutions at times $t=1$, $t=2$, $\ldots$, $t=10$ on the plot and connect them with straight lines.
...
::: {#exr-5.10}
In @fig-5.2 you see the analytic solution at $x(0)=5$ and a slope field for an unknown differential equation.
...
::: {#exr-5.11}
Why would Euler's method overshoot the exact solution in regions where the solution exhibits high curvature?
...
2.  Using your code from part (1), build a plot with the value of $\Delta t$ on the horizontal axis and the value of the associated error on the vertical axis. You should use a log-log plot. Obviously you will need to run your code many times at many different values of $\Delta t$ to build your data set.
...
2.  Modify your code from the previous problem so that it works for this problem. Use `tmax = 200` and an appropriately small time step. Start with initial conditions $x_0(0)=20$ rabbits and $x_1(0)=1$ fox.
...
5.  If you cut your time step in half, what do you see in the two plots? Why? What is Euler's method doing here?
...
1.  Explain the minus sign in the $S'$ equation in the context of the spread of a virus.

2.  Explain the product $SI$ in the $S'$ equation in the context of the spread of a virus.

3.  Find a numerical solution to the system of equations using your `euler()` function. Use the parameters $\beta = 0.4$ and $\beta = 0.04$ with initial conditions $S(0) = 0.99$, $I(0) = 0.01$, and $R(0) = 0$. Explain all three curves in context.
...
b.  Repeat the process outlined in part (a) to approximate the solution to the differential equation at times $t=2, 3, \ldots, 10$. Also record the exact answer at each of these times by noting that the exact solution is $x(t) = 6e^{-0.5t}$.
...
::: {#exr-5.41}
Write a Python function that implements the Runge-Kutta 4 method in one dimension. Test the problem on several differential equations where you know the solution.
...
A static snapshot of the animation applet is shown in @fig-5.5. When you build this animation you will have control over all three parameters. Like we mentioned before, this sort of animation can be a great playground for building insight into the interplay between parameters.

![Snapshot of the ODE animation applet with ipywidgets.](figures/ODE/widgetsnapshot.png){#fig-5.5 alt="Snapshot of the ODE animation applet with ipywidgets."}
:::
...
::: {#exr-5.43}
Modify the animation routine above to simultaneously show the Euler, Midpoint, and RK4 solutions to a differential equation on top of each other. Animate over different values of $\Delta t$ for fixed values of $x_0$ and $t_{max}$.
...
::: {#exr-5.44}
Again we will consider the differential equation $x' = -\frac{1}{3}x + \sin(t)$ but this time we will only be interested in an animation over time.
...
A static snapshot of the resulting animation can be seen in @fig-5.6. The controls for the animation should be familiar from other media players.

![Snapshot of the ODE animation applet with matplotlib animation.](figures/ODE/mplanimationsnapshot.png){#fig-5.6 alt="Snapshot of the ODE animation applet with matplotlib animation."}
:::

------------------------------------------------------------------------

::: {#exr-5.45}
Modify the code from the previous exercise to show faster and slower animations.
...
::: {#exr-5.46}
Modify the `matplotlib.animation` code from @exr-5.44 to use a different differential equation solver.
...
::: {#exr-5.47}
Modify the `matplotlib.animation` code from @exr-5.44 to show the Euler, Midpoint, and RK4 solutions to a differential equation on top of each other for a fixed value of $\Delta t$.
:::
...
4.  Work our the analytic solution for this differential equation (using separation of variables perhaps). Then compare the values that you found in parts (b) and (c) of this problem to values of the analytic solution and values that you would find form the regular (forward) Euler approximation. What do you notice?
...
::: {#exr-5.51}
Test the Backward Euler method from the previous problem on several differential equations where you know the solution.
...
We examined data fitting in [Chapter -@sec-calculus]. The present situation is really not that much different than regular least squares curve fitting.
...
From Calculus you may recall Newton's Law of Cooling: 
\begin{equation}
 \frac{dT}{dt} = -k(T - T_{ambient}) 
\end{equation}
 where $T$ is the temperature of some object (like a cup of coffee), $T_{ambient}$ is the temperature of the ambient environment, and $k$ is the proportionality constant that governs the rate of cooling. This is a classic differential equation with a well known solution.^[If you do not know the solution to Newton's Law of Cooling then take a moment and do the separation of variables to solve for $T(t)$.] In the present situation we do not want the analytic solution, but instead we will work with a numerical solution since we are thinking ahead to where the differential equation may be very hard to solve in future problems. We also do not want to just look at the data and guess an algebraic form for the function that best fits the data. That would be a trap! (why?) Instead, we rely on our knowledge of the physics of the situation to give us the differential equation.

The following data table gives the temperature (degrees $F$) at several times while a cup of tea cools on a table [@Bauer]. The ambient temperature of the room is $65^\circ F$.
...
::: {#exr-5.56}
In the paper [*Steeping Tea: A differential equations approach to a great cup of fruit tea*](https://www.simiode.org/resources/4192/download/1-094-S-SteepingTea-StudentVersion.pdf) [@Bauer], the authors give colour data from photographs of tea that is steeping in a clear mason jar. The temperature data from the previous exercise in this section were taken from this paper.

1.  Read the introduction, methods, and experimental setup in the paper.

2.  Think carefully about the physics of the problem to propose a differential equation (NOT an algebraic function) which would best models the grayscale data found on page 3 of the paper. Your model will likely involve at least one unknown parameter.

3.  Use the least squares data fitting routine outlined in the previous exercise to find the value(s) of your parameter(s) which will create a high quality match between the numerical solution to your ODE and the data.

4.  Plot your solution curve along with the data.
...
::: {#exr-5.57}
#### Village Epidemic
(This exercise is modified from [@Winkel])
...
::: {#exr-5.58}
#### Bedridden Boys Problem
(This problem is modified from [@Miller])
...
::: {#exr-5.61}
Consider the first-order differential equation $x' = f(t,x)$. What is the Midpoint method for approximating the solution to this differential equation? What is the order of accuracy of the Midpoint method? Explain the meaning of the order of the method in the context of solving a differential equation.
...
::: {#exr-5.67}
Explain in clear language how to fit a numerical solution of and ODE model to a dataset.
