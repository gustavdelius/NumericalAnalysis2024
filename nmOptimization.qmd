...
::: {#exr-3.51}
Try to turn your intuitions into algorithms. If $f(x)$ is the function that you are trying to minimize then turn your ideas from the previous exercises into step-by-step algorithms which could be coded. Then try out your codes on the function 
\begin{equation}
f(x) = -e^{-x^2} - \sin(x^2)
\end{equation}
...
::: {#exr-3.54}
Make a plot of the log of the absolute error at iteration $k+1$ agains the log of the absolute error at iteration $k$, similar to @fig-2.4 for several methods and several choices of function. What do you observe?
:::
